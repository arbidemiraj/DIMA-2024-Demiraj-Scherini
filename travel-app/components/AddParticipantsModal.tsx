        ...
        <FlatList
            ...
            renderItem={({ item }) => (
              <View style={{flexDirection: 'row', gap: 15, flexWrap: 'wrap', alignItems:'center', marginTop: 10,}}>
                ...
                <Pressable onPress={() => {if(item.username) OnChangeUser(item.username)}}><Text>{item.username}</Text></Pressable>
                {/* Display other user information as needed */}
                <Pressable onPress={() => addParticipant(item)}><Iconify icon='gala:add' size={22} color={'#000'} style={{alignSelf: 'flex-end'}}/></Pressable>
              </View>
            )}
          />
        </View>
