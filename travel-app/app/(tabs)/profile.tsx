import { StyleSheet, Image, useColorScheme, ActivityIndicator, Button, TouchableOpacity, Dimensions } from 'react-native';
  ...
  const getTrips = async () => {
    ...
    try {
      ...
      if (selected) {
        ...
        query = supabase.from('trip').select(`*, category!inner(*), profile_trip!inner(role, profile!inner(*)), visit!inner(lat, long, description, name, image(*))`);
        query.eq('profile_trip.profile.id', userID).eq('profile_trip.role', 'participant');
      }
