...
export const getCategories = (labelAnnotations: any, addCategories: (categories: string[]) => void) => {
...
export const usePickImage = async (setImage: (imageUri: string) => void, addCategories: (categories: string[]) => void) => {
  try {
    ...
    if (!result.canceled) {
      const compressedImage = await ImageManipulator.manipulateAsync(result.assets[0].uri, [{ resize: { width: 800 } }], { compress: 0.7, format: ImageManipulator.SaveFormat.JPEG });
